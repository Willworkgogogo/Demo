...
require(
    [   
		...
		$.ajax({
				url : 'catagory/all/新能源/1/8/产量',
				type : 'GET',
				beforeSend : function(){
					smm_charts.showLoading(); // map show loading
					setLoad(map.$listInfo_wrap); //list show loading
				},
		...
		$('.map_subnav_title').on('click', function(event) {
			...
			if($(event.target).is('span')){
				...
				$.ajax({
					url : 'catagory/'+$(event.target).text(),
					type : 'GET',
					beforeSend : function(){
						smm_charts.showLoading(); // map show loading
						setLoad(map.$listInfo_wrap); //list show loading
	...
	/*对比栏删除按钮功能实现*/
			...
			if ($(event.target).text() == '删除') {
				for (var i = 0; i < map_company.length; i++) {
					if(map_company[i].name == $(event.target).parent().siblings('.rightCompare_listItems_content').text()){
						...
						map.$smm_map_content_listUl.each(function() {
		...
		checkEvent();
		function checkEvent(){
			map.$smm_map_content_listUl.on('click', function(event){
		...
		function listRender(res){
			...
			for (var i = 0; i < res.data.length; i++) {
				str += ''+'<ul class="smm_map_content_listUl" hello="'+res.data[i].company_info.id+'">'	
						 ...
						 +'<li class="list_seventh">'+res.data[i].product_infos[0].quantity+'</li>'
						 ...
						 +'<li class="list_eighth"><span>'+res.data[i].product_infos[1].quantity+'</span></li>'
			...
			map.$smm_map_content_innerWrap.html(str);
